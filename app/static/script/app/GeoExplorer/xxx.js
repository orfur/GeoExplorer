...
/**
 * api: (define) module = GeoExplorer extends = gxp.Viewer
 */

/**
 * api: constructor .. class:: GeoExplorer(config) Create a new GeoExplorer
 * application.
 * 
 * Parameters: config - {Object} Optional application configuration properties.
 * 
 * Valid config properties: map - {Object} Map configuration object. sources -
 * {Object} An object with properties whose values are WMS endpoint URLs
 * 
 * Valid map config properties: projection - {String} EPSG:xxxx units - {String}
 * map units according to the projection maxResolution - {Number} layers -
 * {Array} A list of layer configuration objects. center - {Array} A two item
 * array with center coordinates. zoom - {Number} An initial zoom level.
 * 
 * Valid layer config properties (WMS): name - {String} Required WMS layer name.
...
var GeoExplorer = Ext
		.extend(
				gxp.Viewer,
				{
					...
					/**
					 * private: property[mapPanel] the :class:`GeoExt.MapPanel`
					 * instance for the main viewport
					 */
					mapPanel : null,
					...
					constructor : function(config) {
						...
						// both the Composer and the Viewer need to know about
						// the viewerTools
						// First row in each object is needed to correctly
						// render a tool in the treeview
						// of the embed map dialog. TODO: make this more
						// flexible so this is not needed.
						config.viewerTools = [
								{
									leaf : true,
					...
					/**
					 * private: method[initPortal] Create the various parts that
					 * compose the layout.
					 */
					initPortal : function() {
						...
						this
								.on(
										"ready",
										function() {
											// enable only those items that were
						...
						// TODO: continue making this Google Earth Panel more
						// independent
						// Currently, it's too tightly tied into the viewer.
						// In the meantime, we keep track of all items that the
						// were already
						...
						googleEarthPanel
								.on(
										...
										function() {
											...
											// now go back and disable all
											// things that were disabled
											// previously
					...
					/**
					 * private: method[createTools] Create the toolbar
					 * configuration for the main panel. This method can be
					 * overridden in derived explorer classes such as
					 * :class:`GeoExplorer.Composer` or
					 * :class:`GeoExplorer.Viewer` to provide specialized
					 * controls.
					 */
					...
					/**
					 * private: method[handleSave] :arg: ``XMLHttpRequest``
					 */
					...
					showUrl : function() {
						...
						/*
						 * var win = new Ext.Window({ title: this.bookmarkText,
						 * layout: 'form', labelAlign: 'top', modal: true,
						 * bodyStyle: "padding: 5px", width: 300, items: [{
						 * xtype: 'label', fieldLabel: this.permakinkText,
						 * readOnly: true, anchor: "100%", selectOnFocus: true,
						 * value: window.location.href }] }); win.show();
						 * win.items.first().selectText();
						 */
					},

					/**
					 * api: method[getBookmark] :return: ``String``
					 * 
					...
					getBookmark : function() {
						...
						// disregard any hash in the url, but maintain all other
						// components
					...
					/**
					 * private: method[displayAppInfo] Display an informational
