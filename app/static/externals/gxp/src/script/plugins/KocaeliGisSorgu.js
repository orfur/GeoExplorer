    ...
    addActions: function() {
        ...
        var actions = [new GeoExt.Action({
            ...
            handler: function(evt)
            {
                ...
                var featuresIlce = this.queryWFSLayerComboBox(this.layers.ilce,"CQL_FILTER=1=1");
                ...
                this.cbx_ilce.on('select', function(box, record, index) {
                    if(index>0){
                        ...
                        var featuresSokaklar = this.queryWFSLayerMahalleComboBox(this.layers.mahalle,"propertyName=uavt_no,mah_adi"+"&CQL_FILTER=ilce_uavt_no="+this.cbx_ilce.getValue().attributes["uavt_no"] );//	INTERSECTS(geometryetry," + this.cbx_mahalle.getValue().geometry.components[0]+ ")");
                        this.getAddress_result(featuresSokaklar);
                        this.queryLayer(this.layers.ilce,"uavt_no=" + this.cbx_ilce.getValue().attributes["uavt_no"],"&propertyName=wkb_geometry",true);
                    }},this);

                this.cbx_mahalle.on('select', function(box, record, index) {
                    if(index>0){
                        this.enumAdresDeger=this.EnumAdres.SOKAK;
                        var featuresSokaklar = this.queryWFSLayerSokakComboBox(this.layers.sokak,"propertyName=uavt_no,sokak_adi"+"&CQL_FILTER=mah_uavt_no="+this.cbx_mahalle.getValue() );//	INTERSECTS(geometryetry," + this.cbx_mahalle.getValue().geometry.components[0]+ ")");
                        this.getAddress_result(featuresSokaklar);
                        this.queryLayer(this.layers.mahalle,"uavt_no=" + this.cbx_mahalle.getValue(),"&propertyName=wkb_geometry",true);
                    }},this);

                this.cbx_sokak.on('select', function(box, record, index) {
                    if(index>0){
                        this.enumAdresDeger=this.EnumAdres.KAPI;
                        var featuresSokaklar = this.queryWFSLayerComboBox(this.layers.kapi,"propertyName=uavt_no,kapi_no"+"&CQL_FILTER=sokak_uavt_no=" + this.cbx_sokak.getValue());//	INTERSECTS(geometryetry," + this.cbx_mahalle.getValue().geometry.components[0]+ ")");
                        this.getAddress_result(featuresSokaklar);
                        this.queryLayer(this.layers.mahallesokak , "uavt_no=" + this.cbx_sokak.getValue(),"&propertyName=wkb_geometry",true);
                    }},this);
                this.cbx_kapi.on('select', function(box, record, index) {
                    if(index>0){
                        this.queryLayer(this.layers.kapi , "CQL_FILTER=uavt_no=" + this.cbx_kapi.getValue(),true);
                    }},this);
...
//			  });
    ...
    queryWFSLayerMahalleComboBox: function(ao_layer,as_cqlFilter)
    {
        ...
        switch (this.enumAdresDeger)
        {
            case this.EnumAdres.ILCE:
                Ext.each(resultValue, function(bilgi){
                    addressDataSet.add(new Ext.data.Record({'ad':bilgi.attributes["ilce_adi"],'kod':bilgi}));
                });
            ...
            case this.EnumAdres.MAHALLE:
                ...
                Ext.each(resultValue, function(bilgi){
                    addressDataSet.add(new Ext.data.Record({'ad':bilgi.attributes["mah_adi"],'kod':bilgi.attributes["uavt_no"]}));
                });
            ...
            case this.EnumAdres.SOKAK:
                ...
                Ext.each(resultValue, function(bilgi){
                    addressDataSet.add(new Ext.data.Record({'ad':bilgi.attributes["sokak_adi"],'kod':bilgi.attributes["uavt_no"]}));
                });
...
//				this.cbx_sokak.store.sort('ad', 'ASC');
            ...
            case this.EnumAdres.KAPI:
                Ext.each(resultValue, function(bilgi){
                    addressDataSet.add(new Ext.data.Record({'ad':bilgi.attributes["kapi_no"],'kod':bilgi.attributes["uavt_no"]}));
                });
                if(addressDataSet.data.items.length>0)
                    addressDataSet.data.items[0].data.ad = " Mahalle Seçiniz";
